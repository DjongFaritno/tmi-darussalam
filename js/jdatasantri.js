...
function setTable() {
	...
	$('#tb_list').DataTable({
		...
		bFilter: false,
		ajax: {
			'url': base_url + "datasantri/load_grid/" + kategori_santri,
			'type': 'GET',
			'data': function (d) {
...
function svSantri() {
	...
	if ($("#add_santri").valid() == true) {
		...
		$.ajax({
			...
			type: "POST",
			url: base_url + "datasantri/simpan_siswa",
...
function AddTOSantri() {
	...
	if ($("#add_santri").valid() == true) {
		...
		$.ajax({
			...
			type: "POST",
			url: base_url + "datasantri/addtosantri/",
			enctype: 'multipart/form-data',
			// dataType:"JSON",
			contentType: false,
			processData: false,
	...
	bootbox.confirm("Anda yakin akan menghapus " + no_registrasi + " ini ?",
		function (result) {
			if (result == true) {
				...
				$.ajax({
					type: "POST",
					url: base_url + "datasantri/DelSantri/" + no_registrasi,
					dataType: "html",
					success: function (data) {
						bootbox.alert({
